...
const gasPriceForTx = Window.GasPriceForTx;
const gasPriceForQuery = Window.GasPriceForQuery;
const gasLimit = Window.GasLimit;

const tokenAmount = Window.TokenAmount;
...
function parkToChainTransaction(dataSource, divID, aStreetName, aSuburb, aCity, aTime1, aTime2) {
    ...
    for (let x = 0; x < numberOfTokens; ++x) {
        ...
        contractWithSigner.approve(parkChainOwnerAddress, 1);

        //console.log(contractWithSigner.estimateGas.addParkingDetails(lenderAddress, aStreetName, aSuburb, aCity, (aTime1 + 1), (aTime1 + (x + 1))));
        //contractWithSigner.addParkingDetails(lenderAddress, aStreetName, aSuburb, aCity, (aTime1 + 1), (aTime1 + (x + 1)));

        //console.log(contractWithSigner.estimateGas.transfer(lenderAddress, 1));
